...
from ldap import initalize, SCOPE_SUBTREE
...
class ADAudit():
    ...
    def get_servers(self, ldap_dict, sever_ous):
        ...
        ldap_obj = initalize(ldap_dict['ldap_url'])
        start = time()
        try:
        ...
        for ou in sever_ous:
            ...
            # Some exception handling.
                ...
                self.log_me.exception(
                    'Size limit exceeded reached when searhcing: %s',
                    ou
                )
...
class WinServerAudit(ADAudit):
    def __init__(self):
        ...
        the following keys: hostname (self explanatory), admins
        (self explanatory).
        self.local_admin_ex = list(), A list of dictionaries containing
        the following keys: hostname (self explanatory), bad_admins
        (self explanatory).
    ...
    def get_local_admins(self):
        ...
        Outputs:
        local_admins - list(), A list of users that are in the server's
    ...
    def get_local_admin_ex(self):
        ...
        Inputs:
        known_admins - list(), A list of known good admins.
        admin_list - list(), The current list of domain admins from AD.
        ...
        for data in self.local_admins:
            ...
            # Matching the hostname to the config file to check against
            # list of approved admins.
            if data['host'] == self.config['local_admins'][data['host']]:
