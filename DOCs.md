...
- **invalid_sources** \- A list() object of log sources thare no longer in active directory.  This variable is populated by the get_log_source_ex method.
...
- **get_log_source_ex** \- This method checks to see if the log sources returned by the WinServerAudit.get_siem_sources method are in Active Directory by comparing them to the list of servers retrieved by the WinServerAudit.get_servers method.  This method populates the WinServerAudit.invalid_sources instance variable.  This method is dependent upon the WinServerAudit.get_siem_sources method and the WinServerAudit.get_servers method.
...
# Iterating through the results.
print('The following log sources are not configured in AD:\n')
for server in win_audit.invlaid_servers:
    print(server)
```